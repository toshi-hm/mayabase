...
1. **ヒーロー**: チャンネル名・キャッチコピー・YouTube / X への CTA ボタン
2. **プロフィール**: 運営者プロフィール(アイコン・自己紹介・リンク)
3. **最新動画(横動画)**: カルーセル(新しい順・最大 6 件・自動切替)
4. **Shorts**: カルーセル(同上、縦型カードで表示)
5. **X ポスト**: カルーセル(新しい順・最大 6 件・自動切替)
6. **フッター**: 各 SNS リンク・コピーライト
...
- ダークベース(YouTube 文化圏に馴染む)+ アクセントカラー 1 色のモダンなトーン。
- CSS 変数(Tailwind v4 `@theme`)でカラー・角丸・余白を一元管理。
