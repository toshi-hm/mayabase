...
2. **気になるテーマは?**(#156): 初回訪問者向けのインタラクティブなテーマ選択導線。
   カテゴリ別に上位動画を紹介し、全件は `/videos/category/{category}/`(#121)へ誘導する
3. **はじめての方へ**(#36): 運営者が手動でキュレーションする代表動画のピックアップ。
   未設定(空配列)の場合はセクションごと非表示にする
4. **人気動画**(#35): カルーセル(再生回数の多い順・最大 6 件)。再生回数は
   `YOUTUBE_API_KEY` 設定時(Data API 経路)のみ取得できるため、1件も取得できて
   いなければセクション自体を非表示にする(`channel-stats.json` の登録者数と同じ
   フォールバック方針)
5. **Shorts**: カルーセル(新しい順・最大 6 件、縦型カードで表示)
6. **X ポスト**: カルーセル(新しい順・最大 6 件・自動切替)
7. **プロフィール**: 運営者プロフィール(画像・自己紹介・リンク)をページ末尾に表示
8. **フッター**: 各 SNS リンク・コピーライト
