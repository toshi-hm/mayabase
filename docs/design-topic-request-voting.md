...
動画リアクションと同様、異なる訪問者同士がほぼ同時に投票した場合の取りこぼし(lost update)はKVの制約上あり得るため対策していない(#533)。投票数は「動画企画の参考にする軽量なシグナル」という位置づけであり、厳密な集計の唯一の根拠にはしないため、この制約を許容している(docs/design-interest-reaction.md参照)。
